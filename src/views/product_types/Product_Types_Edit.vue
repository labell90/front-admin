...
export default {
  ...
  methods:{
    ...mapActions([
      "Module_Product_Types_Edit",
      "Module_Product_Types_Show"
    ]),
    Get_Item(){
      this.Module_Product_Types_Show(this.$route.params.id).then(response => {
    ...
    Edit_Item(){
      this.loading=true;
      this.Module_Product_Types_Edit(this.items).then(response => {
        this.loading=false;
        this.Methods_Notify_Update();
        this.$router.push({name:'product_types_index'});
      }).catch(error => {
...
<template>
  ...
  <q-card>
    ...
    <template v-else>
      ...
      <q-card>
        <q-card-section>
          <global_actions_header_buttons :create="true"  route="product_types"></global_actions_header_buttons>
          <q-separator class="q-mt-xl"/>
          <strong class="text-grey-10">ویرایش نوع محصولات : <span class="text-red-8">{{ items.name }}</span></strong>
