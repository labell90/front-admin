...
export default {
  ...
  methods :{
    ...mapActions([
      ...
      "Module_Customer_Trash_Delete",
      "Module_Customer_Restore",
      "Module_Customer_Trash_Index"
    ...
    Items_Get(per_page,page){
      ...
      this.Module_Customer_Trash_Index({per_page:per_page,page:page}).then(res => {
    ...
    Item_Delete(id){
      ...
      this.Module_Customer_Trash_Delete(id).then(res => {
    ...
    Item_Restore(id){
      this.delete_loading=true;
      this.Module_Customer_Restore(id).then(res => {
        this.items = this.items.filter(item => {
          return item.id !== id;
        })
        this.Methods_Notify_Delete();
...
<template>
  <q-card>
    <q-card-section>
      ...
      <q-btn :to="{name : 'customer_index'}" class="float-right q-mr-sm" color="yellow-9" text-color="dark" icon="fas fa-backward" glossy label="بازگشت"></q-btn>
    </q-card-section>
    <q-card-section>
      <q-table
          flat
