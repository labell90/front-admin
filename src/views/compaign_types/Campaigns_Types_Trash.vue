...
export default {
  ...
  methods :{
    ...mapActions([
      ...
      "Module_Campaign_Types_Action_Trash_Delete",
      "Module_Campaign_Types_Action_Restore",
    ...
    Items_Get(per_page,page){
      ...
      this.Module_Campaign_Types_Action_Index({per_page:per_page,page:page}).then(res => {
    ...
    Item_Delete(id){
      ...
      this.Module_Campaign_Types_Action_Restore(id).then(res => {
...
<template>
  <q-card>
    <q-card-section>
      ...
      <q-btn :to="{name : 'campaigns_types_create'}" class="float-right" color="pink-7"  glossy icon="fas fa-plus-circle" label="افزودن آیتم جدید"></q-btn>
      <q-btn :to="{name : 'campaign_types_index'}" class="float-right q-mr-sm" color="yellow-9" text-color="dark" icon="fas fa-backward" glossy label="بازگشت"></q-btn>
    </q-card-section>
    <q-card-section>
      <q-table
          flat
