...
export default {
  ...
  methods :{
    ...mapActions([
      ...
      "Module_Group_Action_Delete",
      "Module_Group_Action_Activation",
    ]),
    ...
    Item_Delete(id){
      ...
      this.Module_Group_Action_Delete(id).then(res => {
    ...
    Item_Activation(id){
      ...
      this.Module_Group_Action_Activation(id).then(res => {
        this.items = this.items.filter(item => {
          if (item.id === id){
...
<template>
  <q-card>
    <q-card-section>
      ...
      <q-btn :to="{name : 'groups_create'}" class="float-right" color="teal-8"  glossy icon="fas fa-plus-circle" label="افزودن آیتم جدید"></q-btn>
      <q-btn :to="{name : 'groups_trash'}" class="float-right q-mr-sm" color="red-8"  glossy icon="fas fa-trash" label="موارد حذف شده"></q-btn>
    </q-card-section>
    <q-card-section>
      <q-table
          flat
        ...
        <template v-slot:body-cell-tools="props">
          <q-td :props="props">
            <div class="text-center">
              <q-btn :to="{name:'groups_edit',params:{id:props.row.id}}" glossy title="ویرایش آیتم" class="q-ma-xs" color="blue-8" icon="fas fa-edit" size="9px" round  />
