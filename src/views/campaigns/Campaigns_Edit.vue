  ...
  methods:{
    ...
    Get_Item(){
      this.Module_Campaign_Action_Show(this.$route.params.id).then(response => {
        ...
        //convert dates for persian calendar
        if (this.items.start_date){
          this.jalali_start_date = this.Methods_Date_Gregorian_To_Jalali(this.items.start_date)
        }
        if (this.items.end_date){
          this.jalali_end_date = this.Methods_Date_Gregorian_To_Jalali(this.items.end_date)
        }
        if (this.items.desired_end_date){
          this.jalali_desired_end_date = this.Methods_Date_Gregorian_To_Jalali(this.items.desired_end_date)
        }
        this.loading=false;
      }).catch(error =>{
    ...
    <template v-else>
      <q-card>
        <q-card-section>
          ...
          <q-btn :to="{name : 'campaigns_index'}" class="float-right" color="grey-8"  glossy icon="fas fa-arrow-left" label="بازگشت"></q-btn>
          <q-btn :to="{name : 'campaigns_create'}" class="float-right q-mr-sm" color="pink-7"  glossy icon="fas fa-plus-circle" label="افزودن آیتم جدید"></q-btn>
        ...
        <q-card-section>
          <div class="row">
            ...
            <div class="col-12 q-pa-xs">
              <q-btn color="pink-7" :loading="edit_loading" @click="Edit_Item" glossy icon="fas fa-plus-circle" label="ویرایش آیتم"></q-btn>
            </div>
          </div>
