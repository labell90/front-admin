...
export default {
  ...
  methods : {
    ...mapActions([
      "Module_Form_Action_Show",
      "Module_Form_Action_Edit",
    ]),
    Add_Items() {
    ...
    Get_Item(){
      this.Module_Form_Action_Show(this.$route.params.id).then(response => {
    ...
    Edit_Item(){
      ...
      this.Module_Form_Action_Edit(this.items).then(res => {
...
<template>
  ...
  <template v-else>
    <q-card>
      <q-card-section>
        <strong class="text-grey-10">ویرایش فرم : <span class="text-red-8">{{ items.name }}</span></strong>
        <q-btn :to="{name : 'forms_index'}" class="float-right" color="yellow-9" text-color="black" glossy icon="fas fa-arrow-left" label="بازگشت"></q-btn>
        <q-btn :to="{name : 'forms_create'}" class="float-right q-mr-sm" color="teal-8"  glossy icon="fas fa-plus-circle" label="افزودن آیتم جدید"></q-btn>
