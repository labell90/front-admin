  ...
  methods:{
    ...
    Get_Item(){
      this.Module_Product_Show(this.$route.params.id).then(response => {
        ...
        if (this.items.options.length > 0){
          this.items.options.forEach(item => {
        ...
        <q-card-section>
          <div class="row">
            ...
            <div class="col-12 q-pa-xs q-mt-md">
              <q-btn color="blue-7" :loading="loading" @click="Edit_Item" glossy icon="fas fa-edit" label="ویرایش آیتم"></q-btn>
            </div>
          </div>
        </q-card-section>
      </q-card>
