...
export default {
  ...
  methods:{
    ...
    Get_Item(){
      this.Module_Units_Show(this.$route.params.id).then(response => {
    ...
    Edit_Item(){
      this.loading=true;
      this.Module_Units_Edit(this.items).then(response => {
        this.loading=false;
    ...
    <template v-else>
      ...
      <q-card>
        <q-card-section>
          ...
          <strong class="text-grey-10">ویرایش  یکای اندازه گیری : <span class="text-red-8">{{ items.name }}</span></strong>
