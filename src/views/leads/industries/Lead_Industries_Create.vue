...
export default {
  ...
  methods:{
    ...
    ...mapActions([
      "Module_Lead_Industry_Action_Create"
    ]),

    Create_Item(){
      this.loading=true;
      this.Module_Lead_Industry_Action_Create(this.items).then(response => {
        this.loading=false;
...
<template>
  ...
  <q-card>
    <q-card-section>
      <strong class="text-grey-10">افزودن صنعت سرنخ جدید</strong>
      <q-btn :to="{name : 'lead_industries_index'}" class="float-right" color="blue-8"  glossy icon="fas fa-list" label="لیست موارد"></q-btn>
