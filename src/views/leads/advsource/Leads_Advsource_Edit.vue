...
export default {
  ...
  methods:{
    ...mapActions([
      "Module_Lead_Advsource_Action_Edit",
      "Module_Lead_Advsource_Action_Show"
    ]),
    Get_Item(){
      this.Module_Lead_Advsource_Action_Show(this.$route.params.id).then(response => {
    ...
    <template v-else>
      ...
      <q-card>
        <q-card-section>
          ...
          <q-btn :to="{name : 'lead_advsource_create'}" class="float-right q-mr-sm" color="pink-7"  glossy icon="fas fa-plus-circle" label="افزودن آیتم جدید"></q-btn>
