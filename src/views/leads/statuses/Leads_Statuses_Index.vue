...
export default {
  ...
  methods :{
    ...
    Items_Get(per_page,page){
      ...
      this.Module_Lead_Status_Action_Index({per_page:per_page,page:page,params:this.query_params}).then(res => {
    ...
    Searchable_Get(){
      this.Module_Lead_Status_Action_Searchable().then(res => {
...
<template>
  <q-card>
    <q-card-section>
      <q-btn :to="{name : 'lead_statuses_create'}" class="float-right" color="pink-7"  glossy icon="fas fa-plus-circle" label="افزودن آیتم جدید"></q-btn>
      <q-btn :to="{name : 'lead_statuses_trash'}" class="float-right q-mr-sm" color="red-8"  glossy icon="fas fa-archive" label="موارد آرشیو شده"></q-btn>
        ...
        <template v-slot:body-cell-tools="props">
          <q-td :props="props">
            <div class="text-center">
              <q-btn :to="{name:'lead_statuses_edit',params:{id:props.row.id}}" glossy title="ویرایش آیتم" class="q-ma-xs" color="blue-8" icon="fas fa-edit" size="9px" round  />
