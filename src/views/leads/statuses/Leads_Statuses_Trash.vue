...
export default {
  ...
  methods: {
    ...mapActions([
      "Module_Lead_Status_Action_Trash_Index",
      "Module_Lead_Status_Action_Restore_Delete",
      "Module_Lead_Status_Action_Restore",
    ...
    Items_Get(per_page, page) {
      ...
      this.Module_Lead_Status_Action_Trash_Index({per_page: per_page, page: page}).then(res => {
    ...
    Item_Delete(id) {
      ...
      this.Module_Lead_Status_Action_Restore_Delete(id).then(res => {
    ...
    Item_Restore(id) {
      this.delete_loading = true;
      this.Module_Lead_Status_Action_Restore(id).then(res => {
        this.items = this.items.filter(item => {
          return item.id !== id;
        })
        this.Methods_Notify_Delete();
...
<template>
  <q-card>
    <q-card-section>
      ...
      <q-btn :to="{name : 'lead_statuses_create'}" class="float-right" color="teal-8" glossy icon="fas fa-plus-circle"
             label="افزودن آیتم جدید"></q-btn>
      <q-btn :to="{name : 'lead_statuses_index'}" class="float-right q-mr-sm" color="yellow-9" text-color="dark" icon="fas fa-backward" glossy label="بازگشت"></q-btn>
    ...
    <q-card-section>
      ...
      >
        ...
        <template v-slot:body-cell-is_active="props">
          <q-td :props="props">
            <global_actions_activation_item @Set_Ok="Item_Activation(props.row.id)"
                                            :status="props.row.is_active"></global_actions_activation_item>
          </q-td>
        </template>
