...
export default {
  ...
  methods :{
    ...mapActions([
      ...
      "Module_Lead_Utmmedium_Action_Delete",
      "Module_Lead_Utmmedium_Action_Activation",
    ...
    Items_Get(per_page,page){
      ...
      this.Module_Lead_Utmmedium_Action_Index({per_page:per_page,page:page,params:this.query_params}).then(res => {
    ...
    Searchable_Get(){
      this.Module_Lead_Utmmedium_Action_Searchable().then(res => {
        this.searchable = res.data.result
      })
    },
    Item_Delete(id){
      this.delete_loading=true;
      this.Module_Lead_Utmmedium_Action_Delete(id).then(res => {
    ...
    Item_Activation(id){
      ...
      this.Module_Lead_Utmmedium_Action_Activation(id).then(res => {
        this.items = this.items.filter(item => {
          if (item.id === id){
...
<template>
  ...
  <q-card>
    <q-card-section>
      <q-btn :to="{name : 'lead_utmmedium_create'}" class="float-right" color="pink-7"  glossy icon="fas fa-plus-circle" label="افزودن آیتم جدید"></q-btn>
      <q-btn :to="{name : 'lead_utmmedium_trash'}" class="float-right q-mr-sm" color="red-8"  glossy icon="fas fa-archive" label="موارد آرشیو شده"></q-btn>
