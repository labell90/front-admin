...
export default {
  ...
  methods:{
    ...
    Get_Item(){
      this.Module_tags_Show(this.$route.params.id).then(response => {
    ...
    Edit_Item(){
      this.loading=true;
      this.Module_tags_Edit(this.items).then(response => {
        this.loading=false;
        this.Methods_Notify_Update();
        this.$router.push({name:'tags_index'});
      }).catch(error => {
...
<template>
  <q-card>
    ...
    <template v-else>
      <q-card>
        <q-card-section>
          <strong class="text-grey-10">ویرایش تگ ها  : <span class="text-red-8">{{ items.name }}</span></strong>
          <q-btn :to="{name : 'tags_index'}" class="float-right" color="yellow-9" text-color="black" glossy icon="fas fa-arrow-left" label="بازگشت"></q-btn>
          <q-btn :to="{name : 'tags_create'}" class="float-right q-mr-sm" color="pink-7"  glossy icon="fas fa-plus-circle" label="افزودن آیتم جدید"></q-btn>
