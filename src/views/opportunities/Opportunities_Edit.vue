...
export default {
  ...
  methods:{
    ...mapActions([
      "Module_Opportunities_Action_Edit",
      "Module_Opportunities_Action_Show",
      "Module_Customer_Search",
      "Module_Opportunity_Levels_Action_All"
    ]),
    Get_Item(){
      this.Module_Opportunities_Action_Show(this.$route.params.id).then(response => {
    ...
    Edit_Item(){
      this.loading=true;
      this.Module_Opportunities_Action_Edit(this.items).then(response => {
        this.loading=false;
...
<template>
  ...
  <q-card>
    ...
    <template v-else>
      ...
      <q-card>
        <q-card-section>
          <strong class="text-grey-10">ویرایش فرصت فروش : <span class="text-red-8">{{ items.name }}</span></strong>
          <global_actions_header_buttons :create="true"  route="opportunities"></global_actions_header_buttons>
