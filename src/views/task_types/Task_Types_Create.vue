...
export default {
  ...
  methods:{
    ...mapActions([
      "Module_Task_Types_Create"
    ]),
    Create_Item(){
      this.loading=true;
      this.Module_Task_Types_Create(this.items).then(response => {
        this.loading=false;
...
<template>
  ...
  <q-card>
    <q-card-section>
      <strong class="text-grey-10"> افزودن نوع وظیفه جدید</strong>
      <q-btn :to="{name : 'task_types_index'}" class="float-right" color="blue-8"  glossy icon="fas fa-list" label="لیست موارد"></q-btn>
