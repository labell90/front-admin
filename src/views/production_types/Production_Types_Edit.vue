...
export default {
  ...
  methods:{
    ...mapActions([
      "Module_Production_Types_Edit",
      "Module_Production_Types_Show"
    ]),
    Get_Item(){
      this.Module_Production_Types_Show(this.$route.params.id).then(response => {
    ...
    Edit_Item(){
      this.loading=true;
      this.Module_Production_Types_Edit(this.items).then(response => {
        this.loading=false;
        this.Methods_Notify_Update();
        this.$router.push({name:'production_types_index'});
      }).catch(error => {
...
<template>
  <q-card>
    ...
    <template v-else>
      <q-card>
        <q-card-section>
          <strong class="text-grey-10">ویرایش نوع تولید : <span class="text-red-8">{{ items.name }}</span></strong>
          <global_actions_header_buttons :create="true"  route="production_types"></global_actions_header_buttons>
        ...
        <q-card-section>
          <div class="row">
            ...
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 q-pa-xs">
              <q-input
                  :error="this.Methods_Validation_Check(errors,'color_code')" outlined v-model="items.color_code" label="رنگ نوع تولید "
              >
